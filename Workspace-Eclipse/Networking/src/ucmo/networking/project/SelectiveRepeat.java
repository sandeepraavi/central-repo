...
public class SelectiveRepeat extends Applet implements ActionListener, Runnable {
    ...
    Packet sender[]; // sender array - holds the Packets being sent
	...
	if (sender[total_Packet - 1] != null) {
	    ...
	    if (stopCheck) {
	    	output.append("Pooja::in stopcheck");
	...
	if (cmd == "rdt" && nextseqsum < base + window_len) {
	    ...
	    sender[nextseqsum] = new Packet(true, pack_height + ADVANCE_PACKET,nextseqsum);
